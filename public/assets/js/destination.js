...
$(function () {
  ...
  $("#wish").on("click", function (event) {
    event.preventDefault();
    var newdestination = {
      destination_name: $("#search_term").val().trim(),
      travelled: 0,
    };
...
$("#search").click(function (event) {
  ...
  var result = [];
  $("#search_term").val(""); // this is the code that clears out.
  ...
  if (city) {
    var queryURL =
      ...
      city +
      "&count=10&fields=id,name,images,score,snippet&account=ORID4DVT&fields=all&token=9mzqr3x3hr2juisyaz7mfqmsezfooz85";
    var poiURL =
      "https://www.triposo.com/api/20200405/poi.json?location_id=" +
      city +
      "&tag_labels=sightseeing&order_by=-score&count=10&account=ORID4DVT&fields=all&token=9mzqr3x3hr2juisyaz7mfqmsezfooz85";
  }
  $.ajax({
    url: queryURL,
    method: "GET",
